...
exports.showPersonalizationEditor = showPersonalizationEditor;
...
const webviewIconOptions = (0, personalization_1.getFontAwesomeQuickPickIcons)()
    .map((option) => {
...
let editorTemplatePromise;
async function showPersonalizationEditor(options) {
    const panel = vscode.window.createWebviewPanel("shelfyPersonalizationEditor", `Edit Personalization: ${options.label}`, vscode.ViewColumn.Active, {
        enableScripts: true,
...
function buildFontAwesomeSvgMarkup(iconName) {
    const icon = (0, personalization_1.getFontAwesomeIcon)(iconName);
    if (!icon) {
        return undefined;
    }
...
async function sendIconCatalog(webview) {
    const batchSize = 80;
...
async function getEditorHtml(webview, nonce, options) {
    const initialState = {
        label: options.label,
        kind: options.kind,
        color: options.personalization?.color ?? "",
        icon: options.personalization?.icon ?? null,
        iconSvgMarkup: options.personalization?.icon !== undefined
...
//# sourceMappingURL=personalizationEditor.js.map